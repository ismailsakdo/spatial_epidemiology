...
# Filter the point layer for Week 1 to Week 3
expression = "\"Week\" >= 1 AND \"Week\" <= 3"
point_layer.setSubsetString(expression)

# Count points within each polygon for Week 1 to Week 3
counts = {}
...
# Update the CaseW1, CaseW2, and CaseW3 attributes with the counts
field_name_week1 = 'CaseW1'
field_name_week2 = 'CaseW2'
field_name_week3 = 'CaseW3'
counted_polygon_layer.startEditing()
counted_polygon_layer.addAttribute(QgsField(field_name_week1, QVariant.Int))
counted_polygon_layer.addAttribute(QgsField(field_name_week2, QVariant.Int))
counted_polygon_layer.addAttribute(QgsField(field_name_week3, QVariant.Int))
for polygon_feat in counted_polygon_layer.getFeatures():
    polygon_id = polygon_feat.id()
    count_week1 = counts.get(polygon_id, 0)
    count_week2 = counts.get(polygon_id, 0)
    count_week3 = counts.get(polygon_id, 0)
    counted_polygon_layer.changeAttributeValue(polygon_id, counted_polygon_layer.fields().indexFromName(field_name_week1), count_week1)
    counted_polygon_layer.changeAttributeValue(polygon_id, counted_polygon_layer.fields().indexFromName(field_name_week2), count_week2)
    counted_polygon_layer.changeAttributeValue(polygon_id, counted_polygon_layer.fields().indexFromName(field_name_week3), count_week3)
