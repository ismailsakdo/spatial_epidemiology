...
# Filter the point layer for Week 1 and Week 2
expression = "\"Week\" IN (1, 2)"
point_layer.setSubsetString(expression)

# Count points within each polygon for Week 1
counts_week1 = {}
for polygon_feat in polygon_layer.getFeatures():
    ...
    for point_feat in point_layer.getFeatures():
        ...
        if polygon_geom.contains(point_geom):
            count += 1
    counts_week1[polygon_id] = count

# Filter the point layer for Week 2
expression = "\"Week\" = 2"
point_layer.setSubsetString(expression)

# Count points within each polygon for Week 2
counts_week2 = {}
for polygon_feat in polygon_layer.getFeatures():
    polygon_id = polygon_feat.id()
    polygon_geom = polygon_feat.geometry()
...
# Update the CaseW1 and CaseW2 attributes with the counts
...
for polygon_feat in counted_polygon_layer.getFeatures():
    polygon_id = polygon_feat.id()
    count_week1 = counts_week1.get(polygon_id, 0)
    count_week2 = counts_week2.get(polygon_id, 0)
