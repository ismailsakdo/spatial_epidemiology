...
# Calculate counts for each week and year
week_attributes = ['CaseW1', 'CaseW2', 'CaseW3']  # Attribute names for each week
year_attributes = ['CaseY2001', 'CaseY2002', 'CaseY2003']  # Attribute names for each year
for week in range(1, 4):
    for year in range(2001, 2003):
        # Filter the point layer for the current week and year
        expression = "\"Week\" = {} AND \"Year\" = {}".format(week, year)
        point_layer.setSubsetString(expression)

        # Count points within each polygon for the current week and year
        counts = {}
        ...
        # Update the attribute for the current week and year
        week_field_name = week_attributes[week - 1]
        year_field_name = year_attributes[year - 2000]
        attribute_names = [week_field_name, year_field_name]
        counted_polygon_layer.startEditing()
        for attribute_name in attribute_names:
            if not counted_polygon_layer.fields().indexFromName(attribute_name) >= 0:
                # Add attribute field if it doesn't exist
                attribute = QgsField(attribute_name, QVariant.Int)
        ...
        for polygon_feat in counted_polygon_layer.getFeatures():
            ...
            counted_polygon_layer.changeAttributeValue(polygon_id, counted_polygon_layer.fields().indexFromName(week_field_name), count)
            counted_polygon_layer.changeAttributeValue(polygon_id, counted_polygon_layer.fields().indexFromName(year_field_name), count)
        counted_polygon_layer.commitChanges()

print("Attribute calculation completed.")